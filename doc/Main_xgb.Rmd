...
```{r wkdir, eval=FALSE}
set.seed(0)
setwd("~/Desktop/fall2019-proj3-sec2--grp10-master")
...
#k = c(0.3,0.25,0.2,0.15,0.1)
#k = c(0.3,0.25,0.2,0.15,0.1,0.05)
k = c(0.3,0.25)
...
#-------feature selection
features10<-results$optVariables[1:500]
new_dat_train<-subset(dat_train, select=features10)
new_dat_test<-subset(dat_test, select=features10)

feature_dat_train<-cbind(new_dat_train,dat_train['emotion_idx'])
feature_dat_test<-cbind(new_dat_test,dat_test['emotion_idx'])
...
if(run.cv){
  ...
  for(i in 1:length(k)){
    ...
    err_cv[i,] <-cv.function(feature_dat_train, K, k[i])
...
```{r final_train}
tm_train=NA
tm_train <- system.time(fit_train<-train(feature_dat_train,model_best))
...
if(run.test){
  ...
  tm_test <- system.time(pred <- test(fit_train, new_dat_test))
...
library(caret)
confusionMatrix( factor(as.integer(pred),levels = 1:22), feature_dat_test$emotion_idx)
```
