...
```{r wkdir, eval=FALSE}
set.seed(0)
setwd("C:/Users/Lenovo/Downloads/fall2019-proj3-sec2--grp10-master/fall2019-proj3-sec2--grp10-master/doc")
...
```{r}
train_dir <- "C:/Users/Lenovo/Downloads/train_set/train_set/" # This will be modified for different data sets.
...
dat_train=dat_train[,c(predictors(results)[1:500],"emotion_idx")]
dat_test=dat_test[,c(predictors(results)[1:500],"emotion_idx")]
```
...
```{r final_train}
tm_train=NA
tm_train <- system.time(fit_train <- train_rf(dat_train, par_best))
...
if(run.test){
  ...
  tm_test <- system.time(pred <- predict(fit_train,dat_test))
}
```
